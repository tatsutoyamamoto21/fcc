...
def add():
    ...
    cursor = cnx.cursor()
    query = (
        "INSERT INTO fridge_1 (ItemName, ExpDate, IsBestBefore, Portion) "
        f"VALUES ('{name}', '{exp}', '{best_before}', '{portion}');"
    )
...
def get():
    cnx = connect()
    query = "SELECT * FROM fridge_1 ORDER BY ExpDate;"
    ...
    for i in range(len(result)):
        ...
        result[i] = dict(
            zip(["ItemID", "ItemName", "ExpDate", "IsBestBefore", "Portion"], result[i])
        )
...
def delete(key_id):
    cnx = connect()
    cursor = cnx.cursor()
    query = f"DELETE FROM fridge_1 WHERE ItemID = {key_id};"
    try:
        cursor.execute(query)
        cnx.commit()
    except:
...
def edit():
    ...
    query = (
        f"UPDATE fridge_1 "
        f"SET ItemName = '{name}', ExpDate = '{exp}', IsBestBefore = {best_before}, Portion = {portion} "
        f"WHERE ItemID = {key_id};"
    )
    print(query)
